...
const HOW_TO_STEPS = [
  <>
    목록에서 <strong>연습에 쓸 데이터 번호(1~20번)</strong>를 정한 뒤, 해당 행의 <strong>다운로드</strong>로 CSV를
    저장합니다. 파일명 앞 숫자(01~20)가 데이터셋 번호와 같습니다.
  </>,
  <>
...
export default function PracticeGrade2() {
  return (
    <PracticeGradePage
      ...
      heroDescription={
        <>
          아래 CSV는 <strong>수업·연습용</strong>으로 정리한 자료입니다.{' '}
          <strong>데이터셋 번호는 1번부터 20번까지 연속</strong>이며, 파일명 앞 숫자(01~20)와 같습니다. 각 데이터 설명·
          출처는 카드를 펼쳐 확인한 뒤, 폼에 &quot;데이터 출처&quot;를 적을 때 참고하세요. UTF-8(BOM)이라 엑셀에서 한글이
